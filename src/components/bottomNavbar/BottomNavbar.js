...
function BottomNavbar() {
    ...
    return (
        <aside className="z-10 xl:hidden fixed bottom-0 w-screen bg-white ">
            <div className="space-x-4 flex justify-around max-w-lg mx-auto">
                {
                    navbarList.map((data) =>
                        <span
                            className="cursor-pointer flex flex-col transform items-center rounded-lg px-3 py-2 text-gray-600 transition-colors duration-300 hover:bg-gray-100 hover:text-gray-700"
                            href="#"
                        >
                            {data.icon}
